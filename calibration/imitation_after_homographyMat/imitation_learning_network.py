...
def load_imitation_learning_network(input_image, input_data, input_size, dropout):
    ...
    """ Image input vs speed gradient """
    compute_gradient = True
    if compute_gradient:
        predicted_steers = branches[0][0][0]
        predicted_acc = branches[0][0][1]
        predicted_brake = branches[0][0][2]
        # print("predicted steers shape: {}".format(predicted_steers.shape))
        # print(predicted_steers)
        # print("predicted_acc shape: {}".format(predicted_acc.shape))
        # print(predicted_acc)
        # print("predicted_brake shape: {}".format(predicted_brake.shape))
        # print(predicted_brake)
        gradient_steers = tf.gradients(predicted_steers, input_image)
        gradient_acc = tf.gradients(predicted_acc, input_image)
        gradient_brake = tf.gradients(predicted_brake, input_image)

    return branches, gradient_steers, gradient_acc, gradient_brake
