...
def prepWarp_crop_resize(img,h,shape):
	...
	##  The full size of the image is 600x800. Imitation learning model first crop i    t with img = img [115:510,:]
	##  Basically, cropped the very top and bottom for y-axis. And then it resize th    e cropped image which has size (395,800) to (88,200).
	
	## move all of those points off screen 
	imgidx = np.meshgrid(np.arange(img.shape[0]),np.arange(img.shape[1]))
	imgidx = np.array(imgidx)
	imgidx = np.stack((imgidx[0].flatten(),imgidx[1].flatten(),np.ones((imgidx.shape[1]*imgidx.shape[2]),np.int32)))
	out_coord = np.matmul(h,np.float32(imgidx))
