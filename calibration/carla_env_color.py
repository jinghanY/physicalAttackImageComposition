...
from imitation_color.imitation_learning import ImitationLearning

from adversary.shape_color import Shape
...
class CarlaEnv:
    def __init__(self, min_frames, max_frames, intersection, radius,town='Town01_nemesisA',
    ...
    def step(self):
        ...
        self.agent = ImitationLearning(self.town, self.task, self.intersection, self.save_choice,self.avoid_stopping,opt=self.opt,min_frames=self.min_frames,max_frames=self.max_frames)
...
if __name__ == "__main__":
    ...
    env = CarlaEnv(min_frames=min_frames, max_frames=max_frames, intersection=intersection, radius=radius, town=townName,task=task,opt=opt_this,color_option=color_option,save_choice=saveChoice,adversary_name=adversary_name)
