...
def prepWarp(img,h,shape):
	...
	mask = np.logical_and(
				np.logical_and(
    				np.logical_and(inidx[0] >= crop_threshold1,inidx[1] >= crop_threshold1),
    				np.logical_and(inidx[0] < img.shape[1]-crop_threshold,inidx[1] < img.shape[0]-crop_threshold)),inxy[2]>0
    		    )
	inidx = inidx[:,mask]
	oidx = oidx[:,mask]
	return [oidx,inidx]
