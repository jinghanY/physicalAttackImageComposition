...
class Frame():
	def __init__(self, frameNum, pointsAmount, pointsDirSource, pointsDirFrame, angle_range):
	...
	def getPointsCoorInSource(self,pointsDirSource):
		...
		for angle in self.angle_range:
			self.pointsCoorInSource[angle] = initialCoorFull
		

	def getPointsCoorInFrame(self,pointsDirFrame):
		## update self.pointsCoorInFrame
		for i in range(len(pointsDirFrame)):
			...
			for angle in self.angle_range:
				...
				self.pointsCoorInFrame[angle][i] = coordinate
	...
	def getH(self):
		## update self.H
		estimation_thresh = 0.6 # hyperparameter
		for angle in self.angle_range:
