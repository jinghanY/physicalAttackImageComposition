...
from imitation_before_homographyMat.imitation_learning_network import load_imitation_learning_network
import random
import pickle
import shutil

"""custom libraries"""
#from adversary.shape_homo import Shape

class ImitationLearning(Agent):

    def __init__(self, city_name, task, intersection, save_choice,avoid_stopping, trajectory_no, memory_fraction=0.25, image_cut=[115, 510],opt=0,min_frames=0,max_frames=10000):
        ...
        self.dataset_pt = "datasets_"+str(self.trajectory_no)+"/"+task+"/"+self.intersection+"/"
        ...
        self.images_save_path = "out_"+self.intersection+"_"+str(self.trajectory_no)+"/"
        ...
        """ loading adversary """
        #self.adversary = Shape(city_name)
        self.episode_number = 0
        #self.adversary.lines_rotate(self.opt)
    ...
    def _compute_action(self, rgb_image, speed, direction=None):
        ...
        #self.episode_path = "out_"+self.intersection+"/"+"point"+str(self.opt)+"_full/"
        #try:
        ...
        #self.episode_path = "out_"+self.intersection+"/"+"point"+str(self.opt)+"/"
        try:
        	os.makedirs(self.episode_path, exist_ok=True)
        except:
        	pass
       	if self.min_frames <= self.frame_count <= self.max_frames:
       		scipy.misc.imsave(self.clean_frame_pt+str(self.frame_count)+'.png',image_input)
       	
       	# save input: image, speed, direction
       	#if self.min_frames <= self.frame_count <= self.max_frames:
       	#	scipy.misc.imsave(self.clean_frame_pt+str(self.frame_count)+'.png', image_input)
        ...
        with open(input_info_file,"wb") as handle:
        	pickle.dump(input_info_dict,handle,protocol=pickle.HIGHEST_PROTOCOL)
        ...
        steer_dict_file = self.steer_pt + str(self.frame_count)+".pickle"
        ...
        # save output: control info and steer
        control_dict = {}
        control_dict['steer'] = control.steer
        control_dict['throttle'] = control.throttle
        control_dict['brake'] = control.brake
    ...
    def _control_function(self, image_input, speed, control_input, sess):
        ...
        #    """Scaling the images by scale factor"""
        #    scaling = 3
        #    stream_raw = cv2.resize(cv2.cvtColor(raw_image, cv2.COLOR_BGR2RGB), None,fx=scaling, fy=scaling, interpolation = cv2.INTER_CUBIC)

        #    y_res, x_res = stream_raw.shape[0], stream_raw.shape[1]

        #    cv2.putText(stream_raw, "accel: {0:.3f}".format(round(predicted_acc, 3)), (10, y_res-70),
        #                cv2.FONT_HERSHEY_DUPLEX, 0.8, (255,255,255), 1)
        #    cv2.putText(stream_raw, "brake: {0:.3f}".format(round(predicted_brake, 3)), (10, y_res-45),
        #                cv2.FONT_HERSHEY_DUPLEX, 0.8, (255,255,255), 1)
        #    cv2.putText(stream_raw, "steer: {0:.3f}".format(round(predicted_steers, 3)), (10, y_res-20),
        #                cv2.FONT_HERSHEY_DUPLEX, 0.8, (255,255,255), 1)
        #    cv2.putText(stream_raw, "{}".format(self.frame_count), (10, 30),
        #                cv2.FONT_HERSHEY_DUPLEX, 0.8, (255,255,255), 1)

        #    cv2.imshow('Camera', stream_raw)
        #    cv2.waitKey(1)
        #else:
