...
def readFile(fileName):
		...
		if line.startswith("rot="):
			...
			try:
				for i in range(len(line)):
					...
					adversary_paras_this_all = [float(x.split("=")[-1]) for x in adversary]
					adversary_paras_this = adversary_paras_this_all[:4]
					rgb_paras_this = adversary_paras_this_all[4:]
					rgb_paras_this = [x/255. for x in rgb_paras_this]
					adversary_paras.append(adversary_paras_this)
					rgb_adversary_paras.append(rgb_paras_this)
				loss = float(line[-1].strip("\n").split("loss=")[-1])
				losses.append(loss)
			except:
				continue
			parameters.append(adversary_paras)
			rgb_parameters.append(rgb_adversary_paras)
	...
	return parameters, rgb_parameters, losses, iterations
...
def parameter_one_randomGen_color(x,y):
	"""
	Generate a random number between x and y
	precision 0.1 
	"""
...
def para_gen(para_name,step_size):
	if para_name.startswith("rot"):
		...
		return [parameter_one_randomGen(20,100),100.0,step_size]
...
def para_gen_color(para_name):
	if para_name.startswith("r"):
		return parameter_one_randomGen_color(0,255)
	elif para_name.startswith("g"):
		return parameter_one_randomGen(0,255)
	elif para_name.startswith("b"):
		return parameter_one_randomGen(0,255)

def create_init_color(paras_name):
	paras_d = {}
	paras_d["name"] = []
	paras_d["value"] = []
	...
	for i in range(len(paras_name)):
		...
		init_value = para_gen_color(para_name)
		paras_d["name"].append(para_name)
...
class ImitationLearningThreshold():

    def __init__(self, paras_dict, color_dict,epoch,step_size,memory_fraction=0.25, image_cut=[115, 510],LR=0.1,task='right-turn/',intersection="42_47/"):
        self.task = task
        ...
        for adversary_name in self.paras_ds:
        	no_para_training += len(self.paras_ds[adversary_name]["name"]) + 3
        ...
        for adversary_name in color_dict:
        	self.color_d = create_init_color(color_dict[adversary_name])
        	self.color_ds[adversary_name] = self.color_d
        
        try:
        	fileName_input = "out_files/"+task+intersection+str(no_para_training)+".txt"
        	parameters_input, rgb_parameters_input, losses_input, iteration_number = readFile(fileName_input)
        	parameters_input = np.array(parameters_input)
        	rgb_parameters_input = np.array(rgb_parameters_input)
        	losses_input = np.array(losses_input)
        	idx = np.argsort(losses_input)[0]
        	iteration_number = np.array(iteration_number)
        	paras_input = parameters_input[idx]
        	rgb_paras_input = rgb_parameters_input[idx]
        	...
        	paras_names = ["rot","pos","width","length"]
        	norm_factors_default = [180.0, 380.0, 100.0, 70.0]
        	step_factor_default = [step_size,step_size,step_size,step_size]
        	...
        	for adversary_name in self.color_ds:
        		self.color_ds[adversary_name]['value'] = list(rgb_paras_input[ct])
        		ct += 1
        except:
        	pass
